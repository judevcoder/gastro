...
(function () {
    ...
    /*@ngNoInject*/
    function ClientCartController($state, $rootScope,$scope,$timeout,ClientCartService,ClientService, uiDatetimePickerConfig, $translate, $stateParams, moment, appConstant, RestaurantDetailService, $geolocation) {
        var vm = this;
        vm.show = true;
        debugger;
        ...
        function getOrder(orderId) {
            ...
            ClientService.getOrder(orderId).then(function (response) {
                ...
                for (var i = 0; i < vm.order.orders_detail.data.length; i++ ) {
                    if (!vm.order.orders_detail.data[i].menu_list.data.delivered){
        ...
        function checkTableInterval() {
            if (vm.order.delivery == false && vm.order.pick_up == false) {
                var lowestServeTime = vm.orderDetail[0].serve_at;
                var maxServeTime = vm.orderDetail[0].serve_at;
        ...
        function changeOrdersServeAt() {
            if (vm.order.delivery == true || vm.order.pick_up == true) {
                var lowestServeTime = vm.orderDetail[0].serve_at;
